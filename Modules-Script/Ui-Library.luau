if getgenv().DarkHubUI then 
	if game.CoreGui:FindFirstChild("Dark Hub GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "Dark Hub") then
				v:Destroy()
			end
		end
	end
end
getgenv().DarkHubUI = true
...
local T1UIColor = {
	...
	["Logo Image"] = "rbxassetid://127598561744166",
	...
	["Title Text Color"] = Color3.fromRGB(200, 200, 200),
...
Library_Function.Gui.Name = 'Dark Hub GUI'
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,255,255") .. ")\">" .. tostring("Dark Hub") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "Dark Hub"
	...
	uistr.Thickness = 1;
	uistr.Color = Color3.fromRGB(70, 70, 70);
	...
	TopStroke.BackgroundColor3 = Color3.fromRGB(70, 70, 70)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "<font color=\"rgb(" .. tostring(color or "255,255,255") .. ")\">" .. tostring(TitleNameMain or "Dark Hub") .. "</font> " .. tostring(getgenv().MainDesc or "")
	...
	pageControlStroke.Color = Color3.fromRGB(70, 70, 70)
	...
	function Main_Function:CreatePage(cfg)
		if type(cfg) == "string" then
			return self:AddTab(cfg)
		end
		cfg = cfg or {}
		return self:AddTab(cfg.Page_Name or cfg.Page_Title or cfg.Title or "Tab")
	end
	function Main_Function:AddTab(PageName, _Icon)
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    return keybindFunction
        ...
        local pagefunc = {}
        function pagefunc:CreateSection(name)
...
function Library:CreateMain(Setting)
	Setting = Setting or {}
	if not Setting.Image then
		Setting.Image = "rbxassetid://127598561744166"
	end
	getgenv().UIColor["Logo Image"] = Setting.Image
	return Library:CreateWindow(Setting)
end
Library.CreateMain = function(Setting)
	return Library:CreateMain(Setting)
end
Library.CreateNoti = function(Setting)
	Setting = Setting or {}
	Library:Notify({
		Title = Setting.Title or "",
		Description = Setting.Desc or Setting.Description or "",
		Duration = Setting.ShowTime or Setting.Duration or 5
	}, true)
end
getgenv().DarkHubLibrary = Library
return Library
